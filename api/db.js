...
function getDatabase() {
    ...
    // For production/Vercel, you'd want to use a cloud database like Vercel Postgres
    const dbPath = process.env.NODE_ENV === 'production' 
        ? ':memory:' // In-memory for demo, use cloud DB in real production
        : path.join(process.cwd(), 'database/santa-teresa.db');
    ...
    db = new sqlite3.Database(dbPath, (err) => {
        if (err) {
            ...
            // Initialize database if in memory (production demo)
            if (dbPath === ':memory:') {
                initializeDatabase();
            }
        }
...
function initializeDatabase() {
    ...
    db.exec(initSQL, (err) => {
        if (err) {
            ...
            console.log('Database initialized with Santa Teresa 2025-2026 data');
        }
    });
}
